...
func Generate(payload *TokenPayload) (string, time.Time) {
	v, err := time.ParseDuration(config.Config("TOKEN_EXP"))

	if err != nil {
		panic("Invalid time duration. Should be time.ParseDuration string")
	}
	expire := time.Now().Add(v).Unix()
	t := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"exp": expire,
		"ID":  payload.ID,
	})

	token, err := t.SignedString([]byte(config.Config("TOKEN_KEY")))

	if err != nil {
		panic(err)
	}

	return token, time.Now().Add(v)
}
